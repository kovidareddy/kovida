pipeline {
  ...
  stages {
    ...
    stage('GCP Auth') {
        steps {
      withCredentials([usernameColonPassword(credentialsId: 'GCP_CREDENTIALS', variable: 'GCP_CREDENTIALS'), file(credentialsId: 'service_account', variable: 'service_account')]) {
